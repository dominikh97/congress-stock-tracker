...
async function postToWorker(path, payload) {
    ...
    if (!ALERTS_WORKER_URL) {
        return {
            ok: false,
            message: "Email alerts aren't set up yet on this site (ALERTS_WORKER_URL is empty in app.js).",
        };
    }

    try {

        const response = await fetch(`${ALERTS_WORKER_URL}${path}`, {
            method: "POST",
        ...
        return {
            ok: false,
            message: "Couldn't reach the alerts service. Please try again shortly.",
        };
    }
}
...
document.querySelectorAll(".mode-button").forEach(button => {
    ...
    button.addEventListener("click", () => {
        ...
        document.getElementById("alerts-footnote").textContent = isUnsubscribe
            ? "Removing alerts takes effect immediately - no email confirmation needed."
            : "You'll get a verification email - click the link inside and you're set. Nobody but the alerts service ever sees your address.";

        document.getElementById("alerts-submit").textContent =
...
document
    ...
    .addEventListener("submit", async event => {
        ...
        const submitButton = document.getElementById("alerts-submit");
        ...
        try {
            ...
            if (alertsMode === "unsubscribe") {
                ...
                setAlertsStatus("Working...", false);

                result = await postToWorker("/unsubscribe", {
                    email,
                    members,
                    all: unsubscribeAll,
                });
                ...
                setAlertsStatus("Sending verification email...", false);

                result = await postToWorker("/subscribe", { email, members });
            }

            setAlertsStatus(result.message, !result.ok);

            if (result.ok && alertsMode === "subscribe") {
